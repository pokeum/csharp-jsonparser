...
namespace JsonParser.Tests.Unit
{
    public class JsonParserTest
    {
        ...
        public void Ignore_Double_NaN()
        {
            ...
            var jsonString = Json.Serialize(screenInfo);
            _output.WriteLine(jsonString);
            ...
            Assert.Equal(0.0, (double)result["latitude"]);
            Assert.False(result.ContainsKey("longitude"));
            Assert.Equal(0.0, (double)result["altitude"]);
            Assert.Equal(0.0, (double)result["speed"]);
        }
    }
}